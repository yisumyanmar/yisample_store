    ...
    <div class="t_tab">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="logistics_x">
        ...
        <tr>
          ...
          <td>
            <div v-if="!status" >
                <div v-for="(i, index) in hasCityData" :key="index" style="padding-bottom: 15px;">
                    <el-table :data="i" border style="width: 99%;">
                        <el-table-column show-overflow-tooltip prop="title" label="商品名称" width="200"></el-table-column>
                        <el-table-column label="优惠后普通价" width="150" prop="discount">
                          <template slot-scope="scope">
                            <!-- <div v-for="(j, ind) in checkList[index]" :key="ind">
                                <input
                                  @change="getPrice(scope.$index,scope.row.discount[ind],scope.row.id)"
                                  v-model.number="scope.row.discount[ind]"
                                  clearable
                                />
                            </div> -->
                              <input
                    ...
                    <el-table :data="i[0].GoodsSpec" border style="width: 70%;">
                        ...
                        <el-table-column width="170px" prop="discount" label="优惠后普通价">
                            <template slot-scope="scope">
                                <input 
                                    @change="getPriceChild(scope.$index,scope.row.discount,scope.row.id)"
                                    v-model.number="scope.row.discount" clearable />
                            </template>
                        </el-table-column>
                        <!-- <el-table-column width="130px" :prop="i[0].discount[index]" label="操作">
                            <template slot-scope="scope">
                                <input
                                  v-model.number="i[0].discount[index]"
                                  clearable
                                />
                            </template>
                        </el-table-column> -->
        ...
        <tr>
          ...
          <td>
            <span class="notes">
              优惠后普通价 ：￥
              <span style="color: red;">{{(params.discount1).toFixed(2)}}</span> 元
            </span>
            <span class="notes">
              普通价总价 ：￥
              <span style="color: red;">{{(params.total1).toFixed(2)}}</span> 元
            </span>
            <span v-if="!status" class="notes">
              会员价总价 ：￥
              <span style="color: red;">{{(total11).toFixed(2)}}</span> 元
            </span>
          </td>
        </tr>
        <tr>
...
export default {
  ...
  methods: {
    getPrice(index, value, id) {
        ...
        this.discount1 = 0;
        this.total1 = 0;
        // {"title":"","total":0,"discount":0,"goods":[{"goods_id":"4","package_price":1},{"goods_id":"20","package_price":""}]}
        // alert(JSON.stringify(this.params))
        // this.params.goods[index].package_price = value;
        this.getNewPrice();
    },
    getPriceChild(index, value, id) {
        if (this.mark) {
            this.params.goods.push({ goods_id: id, package_price: value });
        } else {
            this.speciValue += value
            this.goods_info1[id].package_price = value;
            let listKey = Object.keys(this.goods_info1);
            for (const i in listKey) {
              if (this.goods_info1[listKey[i]]) {
                if (this.mark) {
                  this.params.goods1.push(this.goods_info1[listKey[i]]);
                } else {
                  this.params.goods1[i] = this.goods_info1[listKey[i]];
                }
              }
            }
        }
        this.params.discount1 = 0;
        this.params.total1 = 0;
        this.discount11 = 0;
        this.total11 = 0;
        this.getNewPrice1();
    },
    getNewPrice() {
      if (!this.status) {
          for (var i in this.hasCityData) {
              // this.params.discount += Number(this.hasCityData[0][i].discount);
              // this.params.total += Number(this.hasCityData[0][i].price_market);
              // this.discount1 += Number(this.hasCityData[0][i].discount1);
              // this.total1 += Number(this.hasCityData[0][i].price_member);
              this.params.discount += Number(this.hasCityData[i][0].discount);
              this.params.total += Number(this.hasCityData[i][0].price_market);
              this.discount1 += Number(this.hasCityData[i][0].discount1);
              this.total1 += Number(this.hasCityData[i][0].price_member);
          }
      } else {
    ...
    getNewPrice1() {
      if (!this.status) {
          for (var i in this.hasCityData) {
              for(var j in this.hasCityData[i][0].GoodsSpec) {
                  this.params.discount1 += Number(this.hasCityData[i][0].GoodsSpec[j].discount);
                  this.params.total1 += Number(this.hasCityData[i][0].GoodsSpec[j].price_market);
                  // this.discount11 += Number(this.hasCityData[i][0].discount1);
                  this.total11 += Number(this.hasCityData[i][0].GoodsSpec[j].price_member);
              }
          }
      } else {
            for (var i in this.updateGoods) {
                this.params.discount1 += Number(this.updateGoods[i].discount);
                this.params.total1 += Number(this.updateGoods[i].price_market);
            }
      }
    },
    queryData() {
    ...
    submitSelect() {
      ...
      for (var i in this.multipleSelection) {
        ...
        if(this.multipleSelection[i].GoodsSpec) {
            for(let j = 0; j < this.multipleSelection[i].GoodsSpec.length; j++) {
                this.goods_info1[this.multipleSelection[i].GoodsSpec[j].id] = {
                  goods_id: this.multipleSelection[i].GoodsSpec[j],
                  package_price: ""
                };
            }  
        }    
      ...
      if (this.status) {
        ...
        for (var i in this.multipleSelection) {
          ...
          let ss = []
          for(let j = 0; j < this.multipleSelection[i].GoodsSpec.length; j++) {
              this.multipleSelection[i].GoodsSpec[j].discount = "0.00"
          }
          ss.push(this.multipleSelection[i])
          this.hasCityData.push(ss);
          let tt = []
          this.checkList.push(tt)
        }
      }
    ...
    deleteItem(id) {
      ...
      for (var i in this.hasCityData) {
        if (this.hasCityData[0][i].id == id) {
          this.hasCityData[0].splice(i, 1);
